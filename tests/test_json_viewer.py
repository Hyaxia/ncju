...
def test_sorting_with_list():
    ...
    assert [node[0] for node in first_level_nodes] == [
        "children",
        "city",
        "name",
        "age",
    ...
    list_items = [
        (node.key, level) for node, level in nodes if level == children_level + 1
    ]
    ...
    list_contents = [
        (node.key, node.size) for node, level in nodes if level == list_level + 1
    ]
